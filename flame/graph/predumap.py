...
class PredictableUMAP(BaseEstimator, TransformerMixin):
    """
    :epkg:`UMAP` is an interesting
    ...
    which approximates the outputs of a :epkg:`UMAP` transformer.
    ...
    def __init__(self, normalizer=None, transformer=None, estimator=None,
        ...
        @param      keep_UMAP_output    if True, keep raw outputs of
        ...
        self.keep_umap_outputs = keep_umap_outputs
    ...
    def fit(self, X, y, sample_weight=None):
        """
        Trains a :epkg:`UMAP` then trains an estimator
        ...
        * `umap_outputs_`: UMAP outputs if *keep_tsne_outputs* is True
        ...
        * `loss_`: loss (:epkg:`sklearn:metrics:mean_squared_error`) between the predictions
        ...
        if self.keep_umap_outputs:
            self.umap_outputs_ = exp if self.normalize else target
        return self
